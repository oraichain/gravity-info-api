...
pub async fn generate_raw_batch_tx(
    ...
    let web3 = Web3::new(&gravity_config.grpc, gravity_config.request_timeout);
    let mut grpc = loop {
        match GravityQueryClient::connect(gravity_config.grpc.to_string()).await {
    ...
    let gravity_bridge_address: EthAddress = match evm_chain_params.bridge_ethereum_address.parse()
    {
        Ok(a) => a,
    ...
    let latest_batches = match get_latest_transaction_batches(&mut grpc, evm_chain_prefix).await {
        Ok(v) => v,
    ...
    let current_valset =
        find_latest_valset(&mut grpc, evm_chain_prefix, gravity_bridge_address, &web3).await;
    ...
    let hash =
        encode_tx_batch_confirm_hashed(evm_chain_params.gravity_id.clone(), target_batch.clone());
    ...
    match encode_batch_payload(
        current_valset,
        &target_batch,
        &sigs,
        evm_chain_params.gravity_id.to_string(),
    ) {
