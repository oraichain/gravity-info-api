    ...
    loop {
        ...
        for block in blocks.into_iter() {
            ...
            for tx in block.data.unwrap().txs {
                let raw_tx_any = cosmos_sdk_proto::Any {
                ...
                let body_any = cosmos_sdk_proto::Any {
                ...
                for message in tx_body.messages {
                    if message.type_url == "/gravity.v1.MsgSendToEth" {
                        ...
                        let msg_send_to_eth_any = cosmos_sdk_proto::Any {
                        ...
                        let msg_ibc_transfer_any = cosmos_sdk_proto::Any {
...
pub fn transaction_info_thread(gravity_config: GravityConfig, db: Arc<DB>) {
    info!("Starting transaction info thread");

    thread::spawn(move || loop {
        let runner = System::new();
        runner.block_on(async {
            match transactions(&gravity_config, &db).await {
                Ok(_) => (),
                Err(e) => {
